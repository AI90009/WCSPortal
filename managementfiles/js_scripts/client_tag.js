...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const campaignForm = document.getElementById('saveTag');
...
const toggleFormFields = (form, state) => {
  ...
  if (campaignForm) {
    campaignForm.addEventListener('submit', function (e) {
      ...
      const practiceName = formData.get('client_tag')?.trim();
  
      if (!practiceName) {
        toastr.error('Please enter a valid client tag to be used');
      ...
      $.ajax({
        url: '../managementfiles/php_scripts/createTag.php',
        method: 'POST',
  ...
  // Initialize DataTable
    ...
    $('.userDatatable').DataTable({
      ajax: {//WCSPortal/managementfiles/json_data/getUser.php
        url: '../managementfiles/json_data/getTags.php',
